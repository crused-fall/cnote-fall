...
* [[黎曼可积的Lebesgue判别]]:第一节当中，我们需要证明黎曼可积能导致函数几乎处处连续。从an epsilon of room的角度出发，等价于证明$$C_r:=\{x\in [a,b]:\omega_f(x)\geq r\}$$是零测度的，对任意$r>0$成立。然后我们需要利用黎曼可积的达布等价条件中$$U(f,P)-L(f,P)=\sum_{k=1}^n (M_k-m_k)|I_k|$$可以随着划分$P$的选取使得上面的和任意小的特性，来寻找对$C_r$的覆盖，并说明这样的覆盖的区间的长度总和是可以任意小的，从而证明集合零测度。
