...
对任意的$t \in K,\varepsilon>0$都存在一个$B_{\delta_{\varepsilon,t}}(t)$,使得任意的$x\in B_{\delta_{\varepsilon,t}}(t)$都有$$d_Y(f(x),f(t))<\varepsilon$$于是$\mathcal{B}_{\varepsilon}:=\{B_{\delta_{\varepsilon,t}}(t):t\in K\}$构成对紧集的一个开覆盖，于是其中存在有限子覆盖，假设为$\{B_{\delta_{\varepsilon,t_1}}(t_1),\cdots,B_{\delta_{\varepsilon,t_n}}(t_n)\}$，于是我们大可以取$$\delta_{\varepsilon}:=\min\{\delta_{\varepsilon,t_1},\cdots,\delta_{\varepsilon,t_n}\}$$这样就摆脱了$\delta$对位置$t$的依赖。但是这时候我们会遇到一个困难:

> [!failure] 当前遇到的一个困难
> 对于任意的$x,y$满足$d_X(x,y)<\delta_{\varepsilon}$的时候，我们想要导出$d_Y(f(x),f(y))<\varepsilon$。但是现在由于$x\in K$那么$x$必然在某个有开覆盖当中，假设$x\in B_{\delta_{\varepsilon,t_i}}(t_i)$不过有限覆盖加上局部条件只能告诉我们$$d_Y(f(x),f(t_i))<\varepsilon$$但是我们依旧不清楚$d_Y(f(x),f(y))$是否可以被很好的控制起来。我们想到可以使用三角不等式$$d_Y(f(x),f(y))\leq d_Y(f(x),f(t_i))+d_Y(f(t_i),f(y))$$可是因为无法确认$y$相对于$t_i$的位置从而无法确定不等式第二项是否可以被很好地控制起来。
...
1. 对任意的$\varepsilon>0$，由于函数在$K$上每个点的局部连续性，存在一个对$K$的开覆盖$\mathcal{B}_{\varepsilon/2}:=\{B_{\delta_{\varepsilon,t}}(t):t\in K\}$使得任意$x\in B_{\delta_{\varepsilon,t}}(t)$都有$d_Y(f(x),f(t))<\varepsilon/2$。显然$\mathcal{B}_{\varepsilon/2}':=\{B_{\delta_{\varepsilon,t}/2}(t):t\in K\}$也是对$K$的一个开覆盖。那么$\mathcal{B}_{\varepsilon/2}$作为紧集的开覆盖自然也是存在有限子覆盖$$\{B_{\delta_{\varepsilon,t_1}/2}(t_1),\cdots,B_{\delta_{\varepsilon,t_n}/2}(t_n)\}$$如图我们现在知道，每一个小尺寸的球实际上外面还嵌套了一个大尺寸的同心球。外层的有限开覆盖$$\{B_{\delta_{\varepsilon,t_1}}(t_1),\cdots,B_{\delta_{\varepsilon,t_n}}(t_n)\}$$也是$K$的有限子覆盖。现在选取$\delta_{\varepsilon}:=\min\{\delta_{\varepsilon,t_1}/2,\cdots,\delta_{\varepsilon,t_n}/2\}$
...
3. 定位了$y$以后利用三角不等式得到$$\begin{aligned}d_Y(f(x),f(y))&\leq d_Y(f(x),f(t_i))+d_Y(f(t_i),f(y))\\&<\varepsilon/2+\varepsilon/2=\varepsilon\end{aligned}$$
#### 1.3 策略调整2：基于Lebesgue's covering lemma

这实际上并不是什么新证明，这完全是对1.2的证明的一种"封装"。

> [!note] 定义1.3.1:Lebesgue number
> 令$\mathcal{A}$是度量空间$X$的一个开覆盖。实数$\lambda$为$\mathcal{A}$的Lebesgue数，如果任意$X$的直径小于$\lambda$的子集$C$，都存在某个$A\in \mathcal{A}$使得$C\subset A$。

* 也就是说，如果一个覆盖$\mathcal{A}$存在对应的Lebesgue数，那么空间里面的子集只要直径足够小，那么一定会被完全包含在某个组成开覆盖$\mathcal{A}$的开集$A$当中。也就是说，不会存在半径特别小，同时又横跨两个组成$\mathcal{A}$的开集这种情况。

> [!note] 引理1.3.2:Lebesgue covering lemma
> 紧度量空间的任意开覆盖必存在对应的Lebesgue数。


对任意的$\varepsilon>0$，由于函数在$K$上每个点的局部连续性，存在一个对$K$的开覆盖$\mathcal{B}_{\varepsilon/2}:=\{B_{\delta_{\varepsilon,t}}(t):t\in K\}$使得任意$x\in B_{\delta_{\varepsilon,t}}(t)$都有$d_Y(f(x),f(t))<\varepsilon/2$。由于Lebesgue's covering lemma，我们知道$\mathcal{B}_{\varepsilon/2}$存在一个对应的Lebesgue数$\lambda_{\varepsilon}$。

那么现在我们可以在Lebesgue数上做文章：只要令$\delta_{\varepsilon}=\lambda_{\varepsilon}$，那么$d_{X}(x,y)<\delta_{\varepsilon}$的意思就是说集合$\{x,y\}$作为一个$X$的直径小于Lebesgue数的子集，必然存在某个$B_{\delta_{\varepsilon/2,t}}(t)$包含着$\{x,y\}$，从而由该集合的定义（局部连续性）我们有$$d_Y(f(x),f(t))<\varepsilon/2,d_Y(f(y),f(t))<\varepsilon/2$$于是$$d_Y(f(x),f(y))\leq d_Y(f(x),f(t))+d_Y(f(y),f(t))<\varepsilon$$于是我们便证明了$f$在$K$上的一致连续性。

* 这里我们没有提到紧集的有限子覆盖，那是因为Lebesgue数的存在性本身，也就是Lebesgue covering lemma就是依靠$K$的紧性得到。
* 实际上1.2节已经给出了对于该问题而言的Lebesgue数存在性的构造性证明了。即便我们不需要Lebesgue's covering lemma，对于$K$的开覆盖$\mathcal{B}_{\varepsilon/2},\mathcal{B}_{\varepsilon/2}'$后者一定存在有限子覆盖$\{B_{\delta_{\varepsilon,t_1}/2}(t_1),\cdots,B_{\delta_{\varepsilon,t_n}/2}(t_n)\}$现在定义$$\lambda_{\varepsilon}:=\min\{\delta_{\varepsilon,t_1}/2,\cdots,\delta_{\varepsilon,t_n}/2\}$$而这实际上就是覆盖$\mathcal{B}_{\varepsilon/2}$的一个Lebesgue数。我们假设$S\subset K$是任意一个半径小于$\lambda_{\varepsilon}$的子集。固定一个$x_0\in S$，那么$S\subset B(x_0,\lambda_{\varepsilon})$，如果我们可以证明后者被包含在开覆盖$\mathcal{B}_{\varepsilon/2}$的某个元素当中，这就证明$\lambda_{\varepsilon}$的确是一个Lebesgue数。首先$x_0$肯定被包含在某个$B_{\delta_{\varepsilon,t_i}/2}(t_i)$中，于是任意$y\in B(x_0,\lambda_{\varepsilon})$都有$$\begin{aligned}d_X(y,t_i)&\leq d_X(y,x_0)+d_X(x_0,t_i)\\&<\lambda_{\varepsilon}+\delta_{\varepsilon,t_i}/2\\&\leq \delta_{\varepsilon,t_i}/2+\delta_{\varepsilon,t_i}/2\\&=\delta_{\varepsilon,t_i}\end{aligned}$$正如上图所示$y$也一定在某个与$x_0$相同的$B_{\delta_{\varepsilon,t_i}}(t_i)$当中，而这正是开覆盖$\mathcal{B}_{\varepsilon/2}$的一个元素。于是通过以上证明，我们构造出了$\lambda_{\varepsilon}$这样一个对于开覆盖$\mathcal{B}_{\varepsilon/2}$的Lebesgue数。
### 2. 周期函数都是一致连续的
