...
> 在***有限测度***空间上，函数列$X_n$是一列可测函数，那么$X_n \xrightarrow{\mu}{X}$等价于任意子列$X_{n(m)}$都存在一个子列的之列$X_{n(m_k)}$几乎处处收敛到$X$
...
> [!note]  Urysohn subsequence principle
