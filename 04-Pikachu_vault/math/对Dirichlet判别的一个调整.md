...
Dirichlet的想法是，如果所有的$a_n-a_{n+1}$都是同号的，那么绝对值就可以打开，这就是一个Telescoping sum （参考[[制造telescoping sum求和或求乘积]])，那么最后求和的结果只有最后一项和第一项，而最后一项极限为0，因此显然极限存在！
...
那么原来的问题在单位圆盘边界上是一致收敛的吗？

首先只要挖掉$z =1$在圆周上的一个任意小的邻域，那么单位圆周边界上剩下的部分，都可以保证 $\sum_{1\leq n\leq N} z^n$是一致有界的,那么根据已知收敛的改造后的Dirichlet判别，目标级数在单位圆周$S^1$的任意去掉$z=1$的闭弧上是一致收敛的。

然后我们现在要证明，在任意包含$z=1$的弧$S'$上，级数非一致收敛。也就是说我们需要证明$$\sup_{z\in S'}\left|\sum_{n\geq N}\frac{(-1)^{\lfloor\sqrt{n}\rfloor}}{n}z^{n}\right|\geq \varepsilon_0\tag{1}$$于是我们现在需要给这种和一个下界估计。

令$N=m^2$，然后我们把从$n\geq N$的正整数的集合拆分为无数个形如$$I_k:=[k^2,(k+1)^2-1]\cap \mathbb{Z}$$的“整数区间”。被求和对象$\frac{(-1)^{\lfloor\sqrt{n}\rfloor}}{n}z^{n}$在$n\in I_k$当中的时候会更简单一些,并且也会方便于我们做总模长的下界估计。这主要是因为之前的经验告诉我们
