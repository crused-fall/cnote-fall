...
因此我们现在需要一个新的方法来产生对$e^r$的有理逼近，并且这种逼近需要足够好，以至于我们可以用于判断其是否为无理数。（什么是好的逼近，代码参考[[证明e的平方是无理数#^cdfb19]]）
...
> buffer = 1000;        (*计算中间过程的时候需要的额外精度*)
> (*buffer一定要足够大，否则图像会看上去误差突然为0，这种结果不可靠！*)
> work = prec + buffer;
> 
> (*生成参与求和的每一项*)
> (*为了避免重复计算k!，以及3^k造成计算资源浪费，这里用FoldList*)
> terms = FoldList[(#1*3)/#2 &, 1, Range[n]];
...
>  FrameLabel -> {"n", "|q_n e^3 - p_n| (log10)"}]
>  
> ```
> 
> 图像为：
> 
> ![[有理数逼近E的三次方的误差对数图.png]]
> 
> 我们可以看到，误差即便在对数图当中也是爆炸式增长的。
