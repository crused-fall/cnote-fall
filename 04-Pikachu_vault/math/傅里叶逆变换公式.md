...
那么上面的这个式子与$\widehat{f}(\xi)$之间又有什么关系呢？我们想到在[[实分析的复办法#2.1 函数的傅里叶变换指数级衰减]]当中我们通过![[矩形区域的路径积分.png]]建立起了一个关系$$\widehat{f}(\xi)=\int_{-\infty}^{+\infty}f(u-ib)e^{-2\pi i(u-ib)\xi}\,du$$考虑到[[和以及积分换序]]，我们可以尝试把这个结果带入到傅里叶逆变换公式的被积分对象当中，然后交换次序。
