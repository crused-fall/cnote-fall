...
> [!question] 问题2.3：某校数学分析考研题目
> 假设$\sum_{n\geq 1} a_n$绝对收敛，并且$p>1/2$，证明下面的级数收敛$$\sum_{n\geq 1} \frac{\sqrt{|a_n|}}{n^p}$$

^abe598
* 此问题的另外一种做法见:[[由一个级数收敛证明另一个有关级数的收敛问题#^ef7e6f]]

直接利用柯西不等式问题就立刻解决了。

> [!question] 问题2.4: 一个复分析问题
> 设复序列$\{a_n\}_{n\geq 0}$满足$\sum_{n\geq 1}n^2|a_n|^2<\infty$证明幂级数$f(z):=\sum_{n\geq 0}a_nz^n$在单位闭圆盘$\overline{D}=\{z\in \mathbb{C}:|z|\leq 1\}$上绝对且一致收敛。

* 一个加强命题是，直接证明$\sum_n |a_n|$收敛，因为这样的话我们借用[Weierstrass M判别](https://en.wikipedia.org/wiki/Weierstrass_M-test)我们就能证明幂级数$f(z)$在单位闭圆盘上绝对且一致收敛。而根据此处的想法$$\sum_n |a_n|=\sum_n |na_n|\frac{1}{n}\leq \left(\sum_n n^2|a_n|^2\right)^{1/2}\left(\sum_n \frac{1}{n^2}\right)^{1/2}<\infty$$因此问题就解决了。
