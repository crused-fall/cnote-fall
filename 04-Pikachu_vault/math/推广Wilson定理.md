...
tags:
  ...
  - 代数基础
---
在[[证明费马小定理#2.1 模p的剩余类$ mathbb{Z}/p mathbb{Z}$关于加法和乘法构成一个域]]的位置补充了“Wilson定理”，即$(\mathbb{Z}/p\mathbb{Z})^{\times}$当中全体元素之积的结果。那么我们会自然的想要知道，这个结果在其他结构上会变成什么样子?
...
2. 如果n是一个偶数，令$\mathbf{a}$为$\frac{n}{2}\text{ mod }n$的剩余类，那么$\mathbf{a}$是一个二阶元。回忆我们在[[证明费马小定理#2.1 模p的剩余类$ mathbb{Z}/p mathbb{Z}$关于加法和乘法构成一个域]]当中证明“wilson定理”时候的操作，如果我们可以确认这就是**此群当中的唯一一个二阶元**，那么问题解决。因为除了单位元$\mathbf{0}$以及二阶元$\mathbf{a}$以外的群当中其余元素都两两配对(互为逆元)。于是最后$S = \mathbf{a}+\mathbf{0}=\mathbf{a}$.
...
结合这个例子以及[[证明费马小定理#2.1 模p的剩余类$ mathbb{Z}/p mathbb{Z}$关于加法和乘法构成一个域]]当中“Wilson定理”也就是$(\mathbb{Z}/p\mathbb{Z})^{\times}$当中的情况，我们隐约感觉，一般情况下的$S$的结果会和群的二阶元有关。
