...
根据[[和的积分估计#1. 和的简单积分估计]]中叙述的内容，我们需要简单三点才能确定这里适合做一个简单的积分估计：
1. 是否单调?
2. 误差是否合适?
3. 积分是否足够简单(或者熟悉)?
...
此方法的具体内容可以参考[[和的积分估计#2.如果函数是光滑的，那么则有更好的估计，即Euler-Maclaurin求和估计]]以及其中的案例。
1. **积分主项**:
...
也就是说常数项$C$是否大于0，取决于最后我们对$A$的估计。其中$$\begin{aligned}|A|&\leq \left|\int_{1}^{\infty}B_3(\{t\})f^{(3)}(t)\,dt\right|\\&\leq \frac{3\zeta(3)}{2\pi^3}\int_{1}^{\infty}|f^{(3)}(t)|\,dt=-\frac{3\zeta(3)}{2\pi^3}\int_{1}^{\infty}f^{(3)}(t)\,dt\\&= \frac{3\zeta(3)}{2\pi^3}\frac{19}{16\sqrt{2}}\end{aligned}$$其中对周期函数$B_3(\{t\})$的上界的估计，可以参考[[和的积分估计#2.如果函数是光滑的，那么则有更好的估计，即Euler-Maclaurin求和估计]]当中Euler-Maclaurin求和估计的部分。因此$$\begin{aligned}C&>2\log(2)-2\text{arsinh}(1)+\frac{9}{16\sqrt{2}}-\frac{1}{6}\frac{3\zeta(3)}{2\pi^3}\frac{19}{16\sqrt{2}}\\&>0.013\end{aligned}$$
因此问题得到解决。
### 3.从结果开始逐项放缩 

> [!tip] 这个做法的核心动机
> 我们要求$S_n>\log(n+1)$。这个时候如果我们想到，[[序列差分的估计蕴含着序列本身的估计]]，那么如果我们可以加强命题得到$$a_{n+1}=\Delta S_n > \log\left(1+\frac{1}{n+1}\right)$$那么立刻就能达到我们的目的。
