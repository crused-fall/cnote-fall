...
* [[已知函数与幂函数的内积估计函数L2范数下界#1. 问题1]]：在这个问题当中，我们知道函数$f$的内积的信息$\langle x^k,f \rangle$的信息，然后需要估计$||f||_{L^2([0,1])}$的下界。
...
在[[已知函数与幂函数的内积估计函数L2范数下界#2. 问题2]]：当中，我们需要确认符合内积条件的连续函数是否存在。我们先把条件放宽到$L^2$当中，然后继续采取同样的策略，现在我们要做的是估计这样的函数的$||f||_{L^2}$。
