...
借用[[从二项式系数到Kummer定理#1. Legendre公式]]当中的p-adic valuation的记号，我们这里要证明组合数没有满足$2\leq \frac{2n}{3}<p\leq n$的素因子，也就是证明：$$\nu_p\left(\binom{2n}{n}\right)=\nu_p(2n!)-2\nu_p(n!)=0$$
...
> [!note] Wolfram当中使用Reduce函数求解不等式的技巧
