...
> [!note] kronecker定理
> 对于给定的无理数$\alpha$，以及$\forall \beta\in\mathbb{R}$，$\forall \varepsilon > 0$都存在正整数$q$以及整数$p$使得$$|q\alpha-\beta-p|<\varepsilon$$
> 

这里我们会好奇，假设我们固定一个$q$，那么此处存在的$p$会是与$q,\alpha,\beta$有怎么样关系的一个整数?其实就是离$q\alpha -\beta$最近的整数，当$\varepsilon>0$足够小的时候，Kronecker定理告诉我们$|q\alpha-\beta-p|<\varepsilon$。也就是说$p$作为一个整数，距离实数$q\alpha -\beta$的距离不足$\varepsilon$，这只有距离此实数最近的那个整数才能做到。
...
> [!question] 与一开始的问题的关系
> 回到“问题1”，我们发现，如果我们可以证明Kronecker定理成立，那么我们可以令$\alpha=\frac{1}{2\pi}$,于是由于这是一个无理数，所以对于$\forall \beta\in(0,1)$以及任意足够小的$\varepsilon$，那么一定存在一个正整数$p_{\varepsilon}>0$以及整数$p_{\varepsilon}$使得$$|q_{\varepsilon}\alpha-\beta-p_{\varepsilon}|<\varepsilon\iff \beta-\varepsilon<q_{\varepsilon}\alpha-p_{\varepsilon}<\beta+\varepsilon$$此处我们取$0<\varepsilon<\min\{\beta,1-\beta\}$,从而可以保证$\beta-\varepsilon>0,\beta+\varepsilon<1$于是$q_{\varepsilon}\alpha-p_{\varepsilon}=\{q_{\varepsilon}\alpha\}$。这便意味着$\eta_n = \left\{\frac{n}{2\pi}\right\}$形成的集合在$(0,1)$当中稠密。


> [!note] Kronecker定理（版本3）
...
* [[利用连续函数的identity theorem证明函数方程#2. 求解函数方程2]]:其中，我们利用$\sqrt{2}\mathbb{Z}+\sqrt{3}\mathbb{Z}$在实数中稠密性，加上连续函数$f$在此集合上取值为常数，从而证明函数在整个实数集上是常数。
...
* 注意这里得到的不过是$$\inf_{n\geq 1}||n\alpha||=0$$而不是说$||n\alpha||$的极限的性质。通过[[模1均匀分布与Weyl判别#2.2 一次多项式，非常数项系数为无理数的序列是模1均匀分布的]]，我们知道$\varepsilon(n\alpha)$是区间$[-1/2,1/2)$当中均匀分布的数列，而$||n\alpha||=|\varepsilon(n\alpha)|$因此实际上这个序列的极限是不存在的。

经过这个证明我们还可以得到下面的推论：

> [!note] Dirichlet定理(版本2)，推论2
