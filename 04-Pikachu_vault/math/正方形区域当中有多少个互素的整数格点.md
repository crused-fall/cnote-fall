...
假设$n =(a,b)$，我们注意到$n=1$这个判断函数可以改写为$\varepsilon(n)$的形式，$$\varepsilon(n):=\begin{cases}1&n=1\\0&\text{otherwise}\end{cases}$$那么此时$$S(N) =\frac{1}{N^2}\sum_{a,b\leq N}\varepsilon(\text{gcd}(a,b))$$
在算术函数当中，我们想到可以把这个东西展开为Dirichlet卷积和的形式:$$\varepsilon=\mu*1\iff \varepsilon(n) =\sum_{d|n}\mu(d)$$
于是我们想到可以利用[[把条件函数展开为新的求和然后换序]]当中的“卷积法”。于是上述求和可以转换为:$$\begin{aligned}S(N)
&= \frac{1}{N^2}\sum_{a,b\leq N;(a,b)=1}1 \\ &=
\frac{1}{N^2}\sum_{a,b\leq N}\varepsilon((a,b))\\&=
